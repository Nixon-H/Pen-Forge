...
["urlgrab"]="web-crawl|Urlgrab|( rm -rf \"$HOME/build-temp/urlgrab\" && timeout $GIT_CLONE_TIMEOUT git clone https://github.com/iamstoxe/urlgrab.git \"$HOME/build-temp/urlgrab\" && cd \"$HOME/build-temp/urlgrab\" && go build -o \"\$GOPATH/bin/urlgrab\" && chmod +x \"\$GOPATH/bin/urlgrab\" )|Extract URLs from web pages|URLs from HTML source"
...
["mantra"]="url-analysis|Mantra|( rm -rf \"$HOME/build-temp/mantra\" && timeout $GIT_CLONE_TIMEOUT git clone https://github.com/Brosck/mantra.git \"$HOME/build-temp/mantra\" && cd \"$HOME/build-temp/mantra\" && sed -i 's|module github.com/brosck/mantra|module github.com/Brosck/mantra|' go.mod && go build -o \"\$GOPATH/bin/mantra\" . )|Find secrets in web pages|API keys, tokens, sensitive info"
